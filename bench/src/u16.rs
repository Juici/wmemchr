...
use crate::input::EMPTY_U16 as EMPTY;
use crate::input::HUGE_U16 as HUGE;
use crate::input::SMALL_U16 as SMALL;
use crate::input::TINY_U16 as TINY;
...
fn define_wmemchr_input(
    ...
    group: &str,
    input: Input<u16>,
    bench: impl FnMut(Search<u16>, &mut Bencher<'_>) + Clone + 'static,
) {
    macro_rules! def {
        ($name:literal, $kind:ident) => {
            if let Some(search) = input.$kind() {
                let name = format!("u16/{}/{}", group, $name);
